...
## 总结  
* 游戏完整度:不要存在任何demo字眼，不要存在任何未开放的字眼，即使这个以后也有可能开放，即使这个可能在某个条件下会开放都不可以，更不要出现任何点击无效的按钮（点击就算有提示也最好不要出现），不要存在任何多余无效内容。不要有任何bug，闪退，卡死。尤其重点是游戏前期体验。减少苹果质疑这很重要。
* 游戏链接，分享，任何跳转接口，5星好评，任何不合适的图片内容（请以小学生标准衡量），审核时候全部屏蔽，等审核通过再打开。
* Ipv6必须兼容，具体方法请自行研究（主要思路就是使用域名，然后socket连接时候类型为ipv6即可，服务器不用搞这个）。测试系统使用最新系统就可以。不强求向下兼容。但是最好兼容
* 付费测试，提交审核前重点测试充值，最好都跑一遍，不要让苹果审核员出现点击充值没反应的情况。钱很重要哇。
* 代码更新，用lua就好了，其它谨慎考虑。JSPatch这类的你懂的。
* 涉及到隐私问题，定位，获取相册等，需要注意的一点是该提示的好好提示，提示得完整（具体是告知在哪里有使用到，这很重要，这块尽可能明显，否则有可能被打回）。  
* 另外说明一点就是审核过程中理论上因为某个原因会被打回，但是却没打回（隐私问题居多）。这不意味着这不会有问题，很可能下次就会被打回来了。这里出现过A，B两个项目组，都有一样的问题，但是A项目组被打回了，B项目组没被打回，这点还是需要注意下（因为审核人员差异问题）。所以还是需要严格按照规定来不要有侥幸心理。     
* 还有使用unity的话要注意下，经历过unity4.7的人都知道，早期弄ipv6会发现本地测试通过，testfight测试通过，偏偏就是上传到苹果审核不通过，结果就是unity底层处理机制出错，需要更新下一个版本。所以切记弄iOS要将unity版本弄到最新（大版本号升不了，至少升小版本号）。说实话li2cpp初期也是总有类似这样的情况。
* 狗年大礼包(2.1)的审核问题，目前可行的是邮件回复审核团队可以解决，后续再观察其它方案（表示被打回N多包）。
...
请确保：  
<u><font color="red">测试 app 是否会发生崩溃、是否存在错误(游戏重点测试新手流程，主线任务，充值，如果有涉及到隐私功能（如定位，获取相册权限等），使用到对应功能也要跑跑)</font></u>
确保所有 app 信息及元数据完整且正确<u><font color="red">（这块提审之前看好一些信息，付费相关的，截图等等有没有都设置好就行） </font></u>  
...
检查 app 是否遵循了其他文稿中的相关指南。<u><font color="red">（具体指南看原文链接，其实就是一些技术文档而已，可以不看。）</font></u>  
### 1. 安全  
...
### 2. 性能
#### 2.1 App 完成度
提交以供审核的 app 应该是最终版本，应包含所有必要的元数据和全功能的 URL；<u><font color="red">占位符文本、空白网站以及其他临时内容应在提交前清除。(请直接在审核过程自己写个代码，屏蔽掉相关网站，跳转链接。等审核通过再打开，不会写就直接屏蔽。并且不要管是否是有效。减少苹果质疑)</font></u>,在提交 app 之前，请务必在设备上对 app 错误和稳定性进行测试；如果您的 app 需要登录，请提供演示帐户信息 (并打开您的后台服务！)。<font color="red"><u>(记得弄个测试账号，而且这个账号注意一点就是不要是完全没跑过得新号，最好就是跑了新手的，部分主要功能已经开放的了。原则上是需要所有系统开放，亲测并不需要，只要保证没开放的时候相关入口隐藏即可，按钮不能出现，更不可点击出现，该系统未开放等字眼，否则苹果会以该游戏非完整版而打回，即使该系统以后也会开放)</u></font>，如果您在 app 中提供了 App 内购买项目，请确保审核人员能够看到这些内容，并确保这些内容处于完整且最新的状态，否则请在审核备注中说明相关原因。请不要将 App Review 视作软件测试服务。我们将拒绝不完整的 app 套装以及会出现崩溃或存在明显技术问题的二进制文件。
#### 2.2 Beta 版测试
...
(iii) 这些 app 不得自动启动或者在启动时包含其他自动运行的代码，不得在未经同意的情况下登录，也不得大量生成在用户退出 app 后仍在未经同意的情况下继续运行的进程。这些 app 不得将图标自动添加到 Dock 中，或在用户桌面上留下快捷方式。<u><font color="red">（关于将图标自动添加到dock中的，如果搞h5游戏可能会需要这样的东西，目前只能提示用户让他到safari操作了）</font></u>  
...
(vii) 这些 app 必须使用 Mac App Store 分发更新；不允许使用其他更新机制。<font color="red"><u>(温馨提示:该出现更新的时候就出现，不该出现更新的时候坚决不出现，最重要的是不要利用更新绕过苹果付费流程，保护费该上交的就上交，低调点。审核过程不要出现更新，无论这个更新是否合法的)</u></font>     
...
通过 IAP 购买的所有点数和游戏货币不得过期，并且您应确保为所有可恢复的 App 内购买项目设计一套恢复机制。(注意有用到微信或者支付宝等，有第三方支付代码的都一律去掉，可能会导致被打回！)</u></font>    
